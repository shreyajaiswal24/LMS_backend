...
def profile(request):
    ...
    staff = User.objects.filter(is_instructor_examiner=True)
    context["staff"] = staff
    return render(request, "accounts/profile.html", context)
...
def edit_staff(request, pk):
    lecturer = get_object_or_404(User, is_instructor_examiner=True, pk=pk)
    if request.method == "POST":
...
class LecturerFilterView(FilterView):
    ...
    queryset = User.objects.filter(is_instructor_examiner=True)
...
def render_lecturer_pdf_list(request):
    lecturers = User.objects.filter(is_instructor_examiner=True)
...
def edit_student(request, pk):
    student_user = get_object_or_404(User, is_trainee=True, pk=pk)
    if request.method == "POST":
